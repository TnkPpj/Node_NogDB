...
class NogDBInfo : public Nan::ObjectWrap {
 ...
 private:
  explicit NogDBInfo(nogdb::DBInfo dbInfo_) : dbInfo(dbInfo_) {}
  ~NogDBInfo() {}
  static NAN_METHOD(New) {
      Nan::MaybeLocal<v8::Object> maybe1 = Nan::To<v8::Object>(info[0]);
      NogTxn* obj = ObjectWrap::Unwrap<NogTxn>(maybe1.ToLocalChecked());
      nogdb::Txn& txn = obj->txn;
      nogdb::DBInfo dbInfo_ ;//= nogdb::Db::getDbInfo(txn);
      NogDBInfo* obj2 = new NogDBInfo(dbInfo_);
      obj2->Wrap(info.This());
      info.GetReturnValue().Set(info.This());
  }
...
class NogClassDescriptor : public Nan::ObjectWrap {
 public:
  static NAN_MODULE_INIT(Init) {
      ...
      Set(target, Nan::New<v8::String>("ClassDescriptor").ToLocalChecked(), tpl->GetFunction());
  }
